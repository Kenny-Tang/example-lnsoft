...
/**
 * 文件上传客户端 - 与FileTransferUploadServiceImpl完全兼容
...
public class FileUploadClient {
    ...
    private final FileTransferUploadServiceGrpc.FileTransferUploadServiceBlockingStub blockingStub;
    private final FileTransferUploadServiceGrpc.FileTransferUploadServiceStub asyncStub;
    ...
    public FileUploadClient(String host, int port, String clientId) {
        ...
        this.blockingStub = FileTransferUploadServiceGrpc.newBlockingStub(channel);
        this.asyncStub = FileTransferUploadServiceGrpc.newStub(channel);
    ...
    public UploadResult uploadFile(String filePath, ProgressCallback progressCallback) {
        ...
        try {
            log.info("Starting upload - File: {}, Size: {} bytes",
            ...
            // 2. 初始化上传
            InitiateUploadResponse initResponse = initiateUpload(file, fileHash);
            ...
            log.info("Upload initiated - UploadId: {}, TotalChunks: {}, AlreadyUploaded: {} ChunkSize {}",
                    uploadId, totalChunks, uploadedChunks.size(), initResponse.getChunkSize());

            // 3. 上传分片
            UploadResult uploadResult = uploadChunks(file, uploadId, initResponse.getChunkSize(),
     ...
     * 初始化上传
     */
    private InitiateUploadResponse initiateUpload(File file, String fileHash) {
        ...
        InitiateUploadResponse initiateUploadResponse = blockingStub.initiateUpload(request);
        log.info("initiateUpload {}", JSON.toJSONString(initiateUploadResponse));
    ...
    private UploadResult uploadChunks(File file, String uploadId, int chunkSize,
        ...
        StreamObserver<UploadChunkRequest> requestObserver = asyncStub.uploadChunk(
            new StreamObserver<UploadChunkResponse>() {
                @Override
    ...
    private void sendChunks(File file, String uploadId, int chunkSize,
        ...
        // 计算需要上传的分片
        Set<Integer> chunksToUpload = new HashSet<>();
        ...
        if (chunksToUpload.isEmpty()) {
            log.info("All chunks already uploaded");
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            ...
            for (int chunkIndex : chunksToUpload) {
                ...
                int currentChunkSize = (int) Math.min(chunkSize, fileSize - chunkOffset);

                // 定位到正确位置
                raf.seek(chunkOffset);

                // 读取分片数据
                int bytesRead = raf.read(buffer, 0, currentChunkSize);
                if (bytesRead != currentChunkSize) {
                    throw new IOException(String.format(
                            "Failed to read chunk %d: expected %d bytes, got %d bytes",
                            chunkIndex, currentChunkSize, bytesRead));
                }
                ...
                log.info("Sending chunk {} - Offset: {}, Size: {}, Hash: {}",
                        chunkIndex, chunkOffset, bytesRead, chunkHash);

                requestObserver.onNext(chunkRequest);

                // 添加小延迟避免服务端过载
                if (chunkIndex % 10 == 0) {
                    LockSupport.parkNanos(1_000_000); // 每10个分片暂停1毫秒
                }
            }

            requestObserver.onCompleted();
            log.info("All required chunks sent - Total: {}, Already uploaded: {}, Sent: {}",
                    totalChunks, uploadedChunks.size(), chunksToUpload.size());
